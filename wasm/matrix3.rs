...
pub unsafe fn matrix3invert(a: *mut f32) -> *mut u8 {
  let det = matrix3determinant(a);
  let a = std::slice::from_raw_parts(a, LEN);

  let ptr = alloc(SIZE);
  let mut mat = Vec::from_raw_parts(ptr as *mut f32, LEN, LEN);

  mat[0] = (a[4] * a[8] - a[5] * a[7]) / det;
  mat[1] = (a[2] * a[7] - a[1] * a[8]) / det;
  mat[2] = (a[1] * a[5] - a[2] * a[4]) / det;
  mat[3] = (a[5] * a[6] - a[3] * a[8]) / det;
  mat[4] = (a[0] * a[8] - a[2] * a[6]) / det;
  mat[5] = (a[2] * a[3] - a[0] * a[5]) / det;
  mat[6] = (a[3] * a[7] - a[4] * a[6]) / det;
  mat[7] = (a[1] * a[6] - a[0] * a[7]) / det;
  mat[8] = (a[0] * a[4] - a[1] * a[3]) / det;
